// Console dev tool for Scramjet proxy with draggable support
(function () {
	...
	function createConsoleUI() {
		...
		button.id = "scramjet-console-btn";
		...
		consoleContainer.id = "scramjet-console-container";
		...
		output.id = "scramjet-console-output";
		...
		input.id = "scramjet-console-input";
		input.type = "text";
		...
		header.addEventListener("mousedown", (e) => {
			isDragging = true;
			dragOffsetX = e.clientX - consoleContainer.getBoundingClientRect().left;
			dragOffsetY = e.clientY - consoleContainer.getBoundingClientRect().top;
		});
		...
		document.addEventListener("mousemove", (e) => {
			...
			consoleContainer.style.left = Math.max(0, Math.min(x, maxX)) + "px";
			consoleContainer.style.right = "auto";
			consoleContainer.style.bottom = "auto";
			consoleContainer.style.top = Math.max(0, Math.min(y, maxY)) + "px";
		});
		...
		input.onkeypress = (e) => {
			if (e.key === "Enter") {
				...
				input.value = "";
				executeCommand(code, output);
			}
		};

		return { button, consoleContainer, output };
	}

	function toggleConsole() {
		const container = document.getElementById("scramjet-console-container");
		if (!container) return;

		isConsoleOpen = !isConsoleOpen;
		container.style.display = isConsoleOpen ? "flex" : "none";

		if (isConsoleOpen) {
			const input = document.getElementById("scramjet-console-input");
			if (input) input.focus();
		}
	}

	function executeCommand(code, output) {
	...
	function addLog(type, message) {
		...
		line.style.color = typeColors[type] || "#ccc";
		line.textContent = `[${timestamp}] [${type.toUpperCase()}] ${message}`;
		line.style.marginBottom = "2px";

		const output = document.getElementById("scramjet-console-output");
		if (output) {
			output.appendChild(line);
			output.scrollTop = output.scrollHeight;
		}
	...
	function init() {
		if (document.body) {
			const { button, consoleContainer } = createConsoleUI();
		...
		var __webpack_modules__ = {
				...
				2443: function (e, t, n) {
					...
					function c(e, t) {
						...
						return (
							l < t.length && c.push({ el: this, handlers: t.slice(l) }),
							c
						);
					}
					((t = {
							...
							return (
								...
								(t.prototype.initTpl = function () {
									...
									("top" === o && (r = "flex-start"),
										e.attr(
											"style",
											"justify-content: ".concat(r, "; align-items: ").concat(i)
										));
								}),
								t
							);
